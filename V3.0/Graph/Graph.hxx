...
template<class V, class C>
bool Graph<V, C>::eulerPath() {

	std::vector<long> marks;
	std::vector<std::vector<bool> > bools;
	for(long i=0;i<this->vertices.size();i++){
		if(this->adjacencyMatrix.find(i) != this->adjacencyMatrix.end()){
				marks.push_back(this->adjacencyMatrix[i].size());
		}else{
			marks.push_back(0);
		}
	}
	...
	if(prueba == 2 || prueba == 0){
		...
		forLoop(0,this->vertices.size()){
			...
			if(this->adjacencyMatrix.find(i) != this->adjacencyMatrix.end()){
				for(long j = 0;j<this->vertices.size();j++){
					if(adjacencyMatrix[i].find(j) != adjacencyMatrix[i].end())
						bools[i].push_back(true);
					else
...
template<class V, class C>
bool Graph<V, C>::euler(std::vector<std::vector<bool> > bools, long suma, long i) {
	if(suma == 0){
		std::cout<<vertices[i];
		return true;
	}
